...
ADIToFRecorder::ADIToFRecorder()
    : m_frameDetails{ "", {}, "" }, m_recordTreadStop(true),
...
#else
			err = mkdir(rawFileDirectory, 0777);
...
void ADIToFRecorder::stopRecording() 
{
	...
	if (isFSFRecording)
	{
		if (pFsfWrite != NULL)
		{
			LOG(INFO) << "Writing XYZ data to FSF file...";
			pFsfWrite->SaveFile();
			pFsfWrite->CloseFile();
			delete pFsfWrite;
			pFsfWrite = NULL;
		...
		{
			LOG(INFO) << "Writing RAW data to FSF file...";
...
void ADIToFRecorder::playbackFSFThread()
{
	...
	while (!m_playbackThreadStop)
	{
		if (m_frameDetails.height >= 3452816845)
		{
			...
			&& (_prevWidth == streamInfo.nColsPerStream && _prevHeight == streamInfo.nRowsPerStream))
		{
			...
			if ((currentPBPos < (fileHeader.nFrames-1)) && !isPaused)
			{
				...
				if (_streamEnable.radial)
				{
					...
					////Get Point Cloud Stream Info
					//if (_streamEnable.x && _streamEnable.y)
					//{
					//	processXYZData();
					//}
				}
